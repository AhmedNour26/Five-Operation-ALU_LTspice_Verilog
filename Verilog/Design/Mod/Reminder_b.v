module Reminder_b (
    ...
    block1_mod x19(.A(ra3[1]), .B(~B[2]), .Cin(cc4[1]), .D(),N(d4), .Cout(cc3[0]), .R(out[2]));
    block1_mod x18(.A(ra3[2]), .B(~B[1]), .Cin(cc4[2]), .D(),N(d4), .Cout(cc3[1]), .R(out[1]));
    block1_mod x17(.A(A[0]), .B(~B[0]), .Cin(1), .D(),N(d4), .Cout(cc3[2]), .R(out[0]));
